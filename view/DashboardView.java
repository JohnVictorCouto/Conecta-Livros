...
// Tela inicial com indicadores e recomendacoes de livros
public class DashboardView extends JPanel {
    ...
    private JPanel criarConteudo() {
        ...
        JLabel subtitulo = new JLabel("VisÃ£o geral do acervo e recomendaÃ§Ãµes do dia");
        ...
        painelStats.add(criarStatCard("Livros no Acervo", lblTotalLivros,  "ðŸ“š", UITheme.STAT_BLUE_BG,   UITheme.STAT_BLUE_FG));
        painelStats.add(criarStatCard("DisponÃ­veis",      lblDisponiveis,  "âœ…", UITheme.STAT_GREEN_BG,  UITheme.STAT_GREEN_FG));
        painelStats.add(criarStatCard("Emprestados",      lblEmprestados,  "ðŸ“¤", UITheme.STAT_RED_BG,    UITheme.STAT_RED_FG));
        painelStats.add(criarStatCard("Estantes",         lblEstantes,     "ðŸ“¦", UITheme.STAT_PURPLE_BG, UITheme.STAT_PURPLE_FG));

        conteudo.add(painelStats);
        conteudo.add(Box.createRigidArea(new Dimension(0, 28)));

        // Cabecalho das recomendacoes
        ...
        JLabel lblRec = new JLabel("RecomendaÃ§Ãµes do Dia");
        ...
        // Grid de cards
        painelCards = new JPanel(new GridLayout(0, 2, 16, 16));
        painelCards.setOpaque(false);
        painelCards.setAlignmentX(LEFT_ALIGNMENT);
        painelCards.setMaximumSize(new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE));
    ...
    private JPanel criarLivroCard(Livro livro, String generoNome) {
        ...
        JTextArea lblTitulo = ComponenteUI.labelMultilinha(
            livro.getTitle() != null ? livro.getTitle() : "Sem tÃ­tulo",
            UITheme.fontBookTitle().deriveFont(16f),
            UITheme.TEXT_PRIMARY,
            58
        );

        String autorAno = (livro.getAuthors() != null ? livro.getAuthors() : "")
            + (livro.getPublishYear() > 0 ? " Â· " + livro.getPublishYear() : "");
        ...
        if (livro.isDisponibilidade()) {
            ComponenteUI.BotaoPrimarioSm btn = new ComponenteUI.BotaoPrimarioSm("Registrar EmprÃ©stimo");
            btn.setPreferredSize(new Dimension(180, 36)); // Botao de acao maior e mais largo
            btn.addActionListener(e -> {
                int r = JOptionPane.showConfirmDialog(this,
                    "Registrar emprÃ©stimo de \"" + livro.getTitle() + "\"?",
            ...
            btn.addActionListener(e -> {
                int r = JOptionPane.showConfirmDialog(this,
                    "Registrar devoluÃ§Ã£o de \"" + livro.getTitle() + "\"?",
                    "Confirmar", JOptionPane.YES_NO_OPTION);
                if (r == JOptionPane.YES_OPTION) {
