...
public class MainFrame extends JFrame {
    ...
    private EditoraView      editoraView;
    ...
    public MainFrame() {
        ...
        painelConteudo.add(generoView,    "generos");
        painelConteudo.add(editoraView,   "editoras");
    ...
    private JPanel criarNavbar() {
        ...
        icLogo.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        icLogo.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) { irParaDashboard(); }
        });

        JLabel txtLogo = new JLabel("Conecta Livros");
        txtLogo.setFont(UITheme.fontAppTitle());
        txtLogo.setForeground(UITheme.TEXT_PRIMARY);
        txtLogo.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        ...
        NavLink lGeneros  = new NavLink("Gêneros",   "generos");
        NavLink lEditoras = new NavLink("Editoras",  "editoras");

        links = new NavLink[]{lEstantes, lListas, lBusca, lGeneros, lEditoras};
        linkAtivo = null;
        ...
        // Separador visual antes de Gêneros e Editoras
        JLabel sep = new JLabel("|");
        sep.setFont(UITheme.fontBody());
        sep.setForeground(UITheme.BORDER);
        sep.setBorder(BorderFactory.createEmptyBorder(0, 4, 0, 4));
        linksPanel.add(sep, 3);

        nav.add(linksPanel, BorderLayout.CENTER);

        // ── Lado direito: botão + Cadastrar Livro ────────────────────────────
    ...
    public void abrirDialogLivro() {
        ...
        d.setVisible(true);
        if (dashboardView != null && d.isSalvo()) dashboardView.recarregar();
    }
