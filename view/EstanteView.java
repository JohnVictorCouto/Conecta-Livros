...
public class EstanteView extends JPanel {
    ...
    private static final String[] COLUNAS = {"ID", "Nome da Estante", "Gênero"};
    ...
    private void filtrar(String t) {
        ...
        preencher(cache.stream().filter(e -> e.getName() != null && e.getName().toLowerCase().contains(lc)).toList());
    }
    ...
    private void abrirFormulario(Estante estante) {
        ...
        form.add(ComponenteUI.labelForm("Gênero *"), gbc);
        ...
        dlg.add(rodapeDlg(dlg, () -> {
            ...
            if (comboGen.getSelectedItem() == null) { aviso("Selecione um gênero."); return; }
    ...
    private void confirmarDelecao(Estante e) {
        ...
        if (r == JOptionPane.YES_OPTION && EstanteDAO.deletar(e.getId())) { carregarDados(); sucesso("Estante excluída."); }
    }
